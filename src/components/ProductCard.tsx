...
export default function ProductCard({ product, teamName }: ProductCardProps) {
  return (
    <Link href={`/product/${product.h}`} className="product-card group bg-white rounded-xl overflow-hidden border border-gray-400 transition-all block">
      ...
      {/* Info */}
      <div className="p-3">
        <h3 className="text-xs font-medium text-gray-700 line-clamp-2 leading-tight mb-2 min-h-[2rem]">{product.t}</h3>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[var(--gold)] font-bold text-sm">CHF {product.p}</span>
          <span className="text-sm text-gray-400 line-through">CHF {(Math.floor(parseFloat(product.p) / 0.79) + 0.90).toFixed(2)}</span>
        </div>
        {/* Category tags */}
        <div className="flex flex-wrap gap-1">
          {product.c.map(cat => (
            <span key={cat} className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">{CATEGORIES[cat] || cat}</span>
          ))}
        </div>
      </div>
    </Link>
